...
genericSiLAMessage.commandRequest(this.messageName, this.paramTypeName, this.paramVals) {
  ...
  for (var index = 0; index < paramTypeName.length; index++) {
    ...
    if (paramType == "Bool") { // Move these back to enums!
      setField(fieldNo, sila.Boolean()
        ..value = paramValue);
    } else if (paramType == "String") {
      setField(fieldNo, sila.String()
        ..value = paramValue);
    } else if (paramType == "Integer") {
      setField(fieldNo, sila.Integer()
        ..value = paramValue);
    } else if (paramType == "Real") {
      setField(fieldNo, sila.Real()
        ..value = paramValue);
...
genericSiLAMessage.commandResponse(this.messageName, this.paramTypeName, this.paramVals){
